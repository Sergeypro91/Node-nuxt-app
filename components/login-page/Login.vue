<template>
  <div class="login-form">
    <div class="login-form__wrapper">
      <form ref="form" class="login-form__local" @submit.prevent="onLogin">
      ...
      <div class="login-form__social">
        ...
        <div class="login-form__social-login">
          <a class="btn btn_social-login btn_google hover" :href="gUrl()">
            <div class="btn__icon">
              <svg
          ...
          <a class="btn btn_social-login btn_facebook hover" :href="fbUrl()">
            <div class="btn__icon">
              <svg
                class="icon icon_white icon_big"
                viewBox="0 0 24 24"
          ...
          <a class="btn btn_social-login btn_vk hover" :href="vkUrl()">
            <div class="btn__icon">
              <svg
                class="icon icon_white icon_big"
                viewBox="0 0 24 24"
...
export default {
  ...
  mounted() {
    this.urlString = document.URL.substring(document.URL.indexOf('#') + 1)

    if (this.urlString.search('thisIsVkRes') > 0) {
      this.vkLogin()
  ...
  methods: {
    async onLogin() {
      try {
        ...
        if (this.$v.$invalid) {
          ...
          await this.$store.dispatch('auth2/login', userFormData)
    ...
    vkUrl() {
      ...
      return `${URL.baseUrl}&client_id=${URL.client_id}&v=${URL.v_api}&response_type=${URL.response_type}&redirect_uri=${URL.redirect_uri}&scope=${URL.scope}&state=thisIsVkRes`
    },
    ...
    async vkLogin() {
      try {
        ...
        this.$store.dispatch('setSubmitStatus', 'PENDING')
        await this.$store.dispatch('auth2/vkSignup', url)
        this.$store.dispatch('setSubmitStatus', 'OK')
    ...
    fbUrl() {
      const URL = {
        ...
        client_id: '224007588831543',
        response_type: 'token',
        redirect_uri: 'http://localhost:3000/'
      }

      return `${URL.baseUrl}&client_id=${URL.client_id}&response_type=${URL.response_type}&redirect_uri=${URL.redirect_uri}&state=thisIsFbRes`
    },
    ...
    async fbLogin() {
      try {
        ...
        this.$store.dispatch('setSubmitStatus', 'PENDING')
        await this.$store.dispatch('auth2/fbSignup', url)
        this.$store.dispatch('setSubmitStatus', 'OK')
    ...
    gUrl() {
      const URL = {
        ...
        client_id:
          '924563082303-l6jf3u16jp7opqr5nvlrsjvc12i8s1tv.apps.googleusercontent.com',
        response_type: 'token',
        scope: 'openid profile email',
        redirect_uri: 'http://localhost:3000/'
      }

      return `${URL.baseUrl}&client_id=${URL.client_id}&response_type=${URL.response_type}&state=thisIsGRes&redirect_uri=${URL.redirect_uri}&scope=${URL.scope}`
    },
    ...
    async gLogin() {
      try {
        ...
        this.$store.dispatch('setSubmitStatus', 'PENDING')
        await this.$store.dispatch('auth2/gSignup', url)
        this.$store.dispatch('setSubmitStatus', 'OK')
